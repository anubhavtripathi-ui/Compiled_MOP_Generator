...
.pill-info { display:inline-flex; align-items:center; gap:6px; background:rgba(0,82,200,0.07); border:1px solid rgba(0,82,200,0.2); border-radius:6px; padding:5px 12px; font-size:0.76rem; color:#003399; margin:3px 0; }
...
def _make_manual_notice_para(doc: Document, item_description: str) -> "docx.oxml.OxmlElement":
    """
    Create a prominently formatted paragraph saying the image/attachment
    could not be inserted and must be added manually.
    """
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(6)
    p.paragraph_format.space_after  = Pt(6)
    run = p.add_run(
        f"⚠ [MEDIA NOT INSERTED — Please add manually: {item_description}]"
    )
...
def _insert_image_para(doc: Document, img_bytes: bytes, ext: str) -> "docx.oxml.OxmlElement":
    """Build a paragraph containing the image and return its XML element."""
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(6)
    p.paragraph_format.space_after  = Pt(6)
    ...
    try:
        ...
        # If the picture can't be added (e.g. EMF), re-raise so caller handles it
        p_elem = p._element
        p_elem.getparent().remove(p_elem)
        raise
    p_elem = p._element
...
def _insert_attachment_para(doc: Document, blob: bytes, ext: str,
                             label: str) -> "docx.oxml.OxmlElement":
    """
    Embed an OLE/package attachment into the document.
    Returns a paragraph XML element with the embedded object.
    For simplicity, we insert a hyperlink-style run as actual file embedding
    requires low-level ZIP manipulation — so we add a clearly styled notice
    with the binary embedded as a named file attachment using python-docx's
    part mechanism.
    """
    # We attach the file as a linked package part
    p = doc.add_paragraph()
    run = p.add_run(f"[ATTACHED FILE: {label}.{ext} — See embedded attachment]")
    run.font.name  = "Calibri"
    run.font.size  = Pt(10)
    run.font.bold  = True
    run.font.color.rgb = RGBColor(0x00, 0x52, 0x7A)
    p_elem = p._element
    p_elem.getparent().remove(p_elem)
    return p_elem
...
def build_mop(
    ...
    sections:       dict,
    today_str:      str,
    media_items:    list,           # list of MediaItem from Activity MOP (may be empty)
    ...
    # Track media injection state
    media_queue  = list(media_items)  # ordered list of MediaItem to inject
    media_idx    = 0                  # next media item to inject
    failed_media = []
    injected_count = 0

    for _, (h_elem, sec_key) in heading_map.items():
        # Remove template boilerplate under this heading
        ...
        # ── Inject content paragraphs — with media insertion for SOP ──
        # We insert in REVERSE order using addnext (each goes right after heading)
        # so final order is preserved.

        if sec_key == "sop" and media_queue:
            # Process SOP paragraphs — look for image placeholders
            # We need forward order, so collect insertion points first
            sop_elems = list(content_elems)  # already in forward order

            # Find placeholder positions
            placeholder_positions = []
            for i, p_elem in enumerate(sop_elems):
                text = "".join(t.text or "" for t in p_elem.findall(".//" + qn("w:t")))
                if IMAGE_PLACEHOLDER_RE.search(text):
                    placeholder_positions.append(i)

            # Insert all elements in REVERSE so addnext preserves forward order
            insert_after = h_elem
            prev_inserted = h_elem

            # We'll do a forward-pass insertion approach instead
            # Build a list of (xml_element, is_placeholder) tuples
            insertion_plan = []
            for i, p_elem in enumerate(sop_elems):
                text = "".join(t.text or "" for t in p_elem.findall(".//" + qn("w:t")))
                if IMAGE_PLACEHOLDER_RE.search(text) and media_idx < len(media_queue):
                    # This paragraph IS a placeholder — replace/augment with media
                    media_item = media_queue[media_idx]
                    media_idx += 1
                    insertion_plan.append(("placeholder", p_elem, media_item, text))
                else:
                    insertion_plan.append(("para", p_elem, None, text))

            # Insert in reverse (addnext trick)
            for entry in reversed(insertion_plan):
                kind = entry[0]
                p_elem   = entry[1]
                media_item = entry[2]
                orig_text  = entry[3]

                if kind == "placeholder" and media_item is not None:
                    # Try to insert the media
                    if media_item.kind == "image":
                        try:
                            img_p_elem = _insert_image_para(doc, media_item.blob, media_item.ext)
                            h_elem.addnext(img_p_elem)
                            media_item.injected = True
                            injected_count += 1
                            # Also add a small caption-style para after image
                            ...
                            cap_t.text = f"[Screenshot/Image — copied from Activity MOP]"
                            cap_r.append(cap_t)
                            cap_p.append(cap_r)
                            h_elem.addnext(cap_p)
                        except Exception as ex:
                            media_item.inject_error = str(ex)
                            desc = f"Image #{media_item.position_index + 1} (context: {media_item.context_text[:50] or 'no context'})"
                            failed_media.append(desc)
                            notice = _make_manual_notice_para(doc, desc)
                            h_elem.addnext(notice)
                    else:
                        # OLE / attachment
                        try:
                            att_label = f"Attachment_{media_item.position_index + 1}"
                            att_p = _insert_attachment_para(
                                doc, media_item.blob, media_item.ext, att_label
                            )
                            h_elem.addnext(att_p)
                            media_item.injected = True
                            injected_count += 1
                        except Exception as ex:
                            media_item.inject_error = str(ex)
                            desc = f"Attachment #{media_item.position_index + 1} (.{media_item.ext})"
                            failed_media.append(desc)
                            notice = _make_manual_notice_para(doc, desc)
                            h_elem.addnext(notice)
                else:
                    # Regular paragraph — clone and insert
                    h_elem.addnext(_clone_para(p_elem))

        else:
            # Non-SOP section — just clone and insert in reverse
            for p_elem in reversed(content_elems):
                h_elem.addnext(_clone_para(p_elem))

    # ── Any remaining media items not matched to placeholders ───
    # These are appended at the END of the SOP section as-is
    remaining_media = [m for m in media_queue[media_idx:]]
    if remaining_media:
        # Find the last inserted SOP paragraph to append after
        sop_heading = None
        for child in list(body):
            if child.tag.split("}")[-1] != "p":
                continue
            ...
            if se is None or se.get(qn("w:val"), "") != "Heading1":
                continue
            text = "".join(r.text or "" for r in child.findall(".//" + qn("w:t"))).strip()
            if normalize_heading(text) == "sop":
                sop_heading = child
                break

        if sop_heading is not None:
            # Walk forward to find insert point (after last SOP content)
            insert_after = sop_heading
            for child in list(body):
                if child is sop_heading:
                    found = True
                    continue
                if not found:
                    continue
            ...
            for m in remaining_media:
                if m.kind == "image":
                    try:
                        img_p = _insert_image_para(doc, m.blob, m.ext)
                        insert_after.addnext(img_p)
                        insert_after = img_p
                        m.injected = True
                        injected_count += 1
                    except Exception as ex:
...
with col_right:
    ...
    st.markdown("""
    <div class="footer">
      Smart MOP Generator v6 &nbsp;·&nbsp; Ericsson Internal Tool &nbsp;·&nbsp;
