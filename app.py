...
class MediaItem:
    """Represents one image or OLE attachment extracted from the Activity MOP."""
    def __init__(self, kind, blob, ext, rId, position_index,
                 context_text="", filename="", prog_id=""):
        self.kind           = kind          # "image" | "attachment"
        self.blob           = blob          # raw bytes
        self.ext            = ext           # "png","jpg","xlsx","txt","bin" etc.
        self.rId            = rId
        self.position_index = position_index
        self.context_text   = context_text  # paragraph text just before this item
        self.filename       = filename      # original filename if recoverable
    ...
    def display_name(self):
        ...
        if self.prog_id == "Excel.Sheet.12":
            return f"Excel_Attachment_{self.position_index+1}.xlsx"
        return f"Attachment_{self.position_index+1}.{self.ext}"

    @property
    def is_ole_package(self):
        """True for embedded files (txt, xlsx, docx etc.), False for screenshots."""
        return self.kind == "attachment"


def _extract_package_filename(blob: bytes) -> str:
    """
    OLE2 Package object stores the original filename as a null-terminated
    Latin-1 string near the start of the stream data.
    Try to extract it; fall back to scanning for recognisable filenames.
    """
    try:
        import re
        decoded = blob.decode("latin-1", errors="replace")
        # Scan for strings that look like filenames with known extensions
        matches = re.findall(r'[\x20-\x7E]{4,200}', decoded)
        for m in matches:
            m = m.strip()
            if re.search(r'\.(txt|xlsx|xls|docx|doc|pdf|zip|log|csv|bin)$',
                         m, re.IGNORECASE):
                # Prefer short names (not full paths)
...
def extract_media_from_activity_mop(mop_bytes: bytes) -> list:
    ...
    in strict document order (paragraph walk).

    Strategy:
    - Each paragraph is walked once.
    - Images  : found via <a:blip r:embed="rIdN"> — only real screenshots
                (PNG/JPG). EMF/WMF icon images that accompany an OLE object
                in the SAME paragraph are SKIPPED (they are just the icon).
    - OLE/pkg : found via <w:object> children that carry an r:id attribute
                pointing to oleObject or package relationships.
    - Each paragraph may contain BOTH an icon image AND an OLE object
      (that is how Word embeds files). We only register the OLE object once
      for that paragraph and skip the accompanying EMF icon image.
    """
    ...
    # ── Build full relationship map ──────────────────────────────
    img_rel_map  = {}   # rId → (blob, ext)   for image relationships
    att_rel_map  = {}   # rId → (blob, ext, filename, prog_id)  for OLE/package
    ...
    for rId, rel in doc.part.rels.items():
        ...
        try:
            if rt == "image":
                ...
                ext = "jpg" if raw_ext in ("jpeg",) else raw_ext
                img_rel_map[rId] = (rel.target_part.blob, ext)
                ...
                if rt == "package":
                    # target_ref is the actual filename e.g. "Microsoft_Excel_Worksheet.xlsx"
                    fname = target.split("/")[-1]
                    ext   = fname.split(".")[-1].lower() if "." in fname else "bin"
                    # For xlsx package, ext is already correct
                else:
                    # oleObject .bin — try to extract original filename
                    fname = _extract_package_filename(blob)
                    if fname:
                        ext = fname.split(".")[-1].lower() if "." in fname else "bin"
                    else:
                        ext = "bin"
    ...
    for para in paras:
        ...
        # Collect OLE object rIds in this paragraph
        ole_in_para = set()
        # Also collect the EMF icon rIds that accompany each OLE (to skip them)
        icon_rids_in_para = set()

        for obj in p_xml.findall(f".//{{{_W}}}object"):
            # The OLE data rId comes from <o:OLEObject r:id="...">
            # The icon image rId comes from <v:imagedata r:id="...">
            for child in obj.iter():
                tag = child.tag.split("}")[-1] if "}" in child.tag else child.tag
                rid = child.get(f"{{{_R}}}id")
                if not rid:
                    continue
                if tag in ("OLEObject",) and rid in att_rel_map:
                    # Get ProgID from same element
                    prog_id = child.get("ProgID", "")
                    ole_in_para.add((rid, prog_id))
                elif tag in ("imagedata",) and rid in img_rel_map:
                    icon_rids_in_para.add(rid)

        # Register OLE attachments first (they "own" this paragraph)
        for ole_rid, prog_id in ole_in_para:
            blob, ext, fname = att_rel_map[ole_rid]

            # Override ext/fname for known ProgIDs
            if prog_id == "Excel.Sheet.12" and not fname:
                fname = f"Excel_Worksheet_{position+1}.xlsx"
                ext   = "xlsx"
            elif prog_id == "Excel.Sheet.12" and fname:
                ext = "xlsx"
        ...
        # Register real screenshot images — skip EMF icons and icon rIds
        if not ole_in_para:   # only process images if no OLE in this paragraph
            for blip in p_xml.findall(f".//{{{_BLIP}}}blip"):
                embed = blip.get(f"{{{_R}}}embed")
                if not embed or embed not in img_rel_map:
                    continue
                blob, ext = img_rel_map[embed]
                # Skip EMF/WMF — these are almost always OLE icons or vector art
                if ext.lower() in ("emf", "wmf"):
                    continue
                # Skip very small images (< 2 KB) — likely decorative icons
                if len(blob) < 2000:
                    continue
                known = {m.rId for m in media_items}
                if embed in known:
                    continue
                item = MediaItem(
